/* 成都五日游 · 两阶段（同 case，删掉「方案选择」那步）
   = travel_chengdu_5d 去掉 InspoFlow 方案卡 + TTS-1b + "就第一个" → 澄清确认后直接出体验段落轮播再钻取。
   ①澄清确认 → ②方案生成(体验段落轮播)+展示/钻取。口播文案与三阶段版一致，音频复用。 */
(function (g) {
  ...
  g.LOONA_CASES['travel_chengdu_5d_2stage'] = {
    ...
    events: [
      ...
      /* ===== 阶段② 方案生成 → 体验段落轮播 + 展示/钻取（无选方案，直接开排） ===== */
      { t: 300, gap_ms: 300, comp: 'agent_step', internal: true, label: 'PLANNER · 合槽 + 切段',
        decision: '带爸妈→不爬高、留歇脚、不排太满；直接按巴适美食切 3 段体验：市井烟火/茶馆慢生活/周边透气。', fields: ['带爸妈', '3段·按体验切'] },
